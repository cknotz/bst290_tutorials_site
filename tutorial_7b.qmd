...
The correct statistic for such a relationship is of course *Pearson's correlation coefficient* [@Solbakken2019, chapter 4.5]. In this tutorial, you will learn how you can calculate the correlation coefficient and test for its statistical significance (i.e., how confident you can be that the correlation coefficient you find in your data is different from, greater, or lower than 0) in `R`.
...
The `cor.test()` function allows you to run the significance test that is explained in Solbakken [-@Solbakken2019, chhapter 5.10.2; see also @Kellstedt2018, 183-184]. To use this function, you just specify the two variables you want to use. It will take care of the `NA`s by itself.
...
And, since you have now learned about two other statistical tests and their interpretation, you should know what to look for next: the *p*-value! Based on this result, can we reject the null hypothesis [see also @Solbakken2019, chapter 5.10; @Kellstedt2018, chapter 8]? As in the previous tutorial on the *t*-test, you can use `format.pval()` to translate the *p*-value into something that is a bit easier to interpret:
```{r}
...
* `t` is the *t* statistic. As explained in Solbakken [-@Solbakken2019, chapter 5.10; see also @Kellstedt2018, 183], we use the *t*-test in this case to test if the correlation coefficient is equal to 0 or not;
...
As mentioned in [Tutorial 4](tutorial_4.qmd), a second (and superior) way to plot relationships between quasi-numeric 0-10 scale variables is to use `geom_count()`. This [mini-tutorial](https://kodehjornet.github.io/posts/geom_count/){.external target="_blank"} by Rosa Ottestad and Normal Sunde explains how you do that.
