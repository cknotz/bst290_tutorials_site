...
This is where you would use summary statistics like the mean ("average"), the median, or others [see also @Solbakken2019, chapter 3].
...
Here, `Min.` means "Minimum", `1st Qu.` means "First Quartile", `Median` and `Mean` are obvious, `3rd Qu.` means "Third Quartile", and `Max.` means "Maximum". If you read Solbakken [-@Solbakken2019, chapter 3], then you should know how to interpret these different statistics.
...
Can you interpret each of the statistics shown [as before, see @Solbakken2019, chapter 3].
...
1. In `RStudio`, navigate to the *Tutorial* tab in the upper-right corner of your screen (see the screenshot below).^[If there are no tutorials called *"De-bugging exercises:..."* shown, just restart `R` by clicking on "Session" in the menu at the top of your screen, and there on "Restart R". You may also have to install the `learnr` package --- in that case, `RStudio` will let you know and you only have to do this once.] 
2. Start the interactive exercise for this tutorial (*"De-bugging exercises: Getting to know your data"*). 
3. To make the window larger, use the little button next to the house button; see again the screenshot below.
4. Follow the instructions.

![Finding the de-bugging tutorials](debugex_screen.jpg)
